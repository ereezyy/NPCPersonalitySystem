...
class NPC:
    ...
    def interact(self, entity_id, action, context=None):
        ...
        if action == "greet":
            ...
            imp = 0.1
            if self.memory.will_remember(imp):
                self.memory.add_event(f"Was greeted by {entity_id}", importance=imp, tags=["interaction", entity_id])
            ...
            imp = 0.9
            if self.memory.will_remember(imp):
                self.memory.add_event(f"Was attacked by {entity_id}", importance=imp, tags=["combat", "negative", entity_id])
            ...
            imp = 0.4
            if self.memory.will_remember(imp):
                self.memory.add_event(f"Received a gift from {entity_id}", importance=imp, tags=["interaction", "positive", entity_id])
