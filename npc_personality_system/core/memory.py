...
class Memory:
    ...
    def add_event(self, description, importance=0.5, tags=None):
        ...
        if len(events) >= self.capacity:
            # EXPLICIT GUARD CLAUSE: We check if the new event's importance is lower than the
            # current minimum element in the heap (events[0][0]). If it is, we discard it immediately.
            # This guarantees that any item proceeding past this point will be strictly >= the minimum
            # element, making it safe to use heapq.heapreplace later without evicting higher-priority memories.
        ...
        if len(events) >= self.capacity:
            ...
            # If at capacity and we've reached here, we know the new item will replace an existing item
            # (since we checked importance < events[0][0] earlier).
            # We use heapreplace which is slightly more efficient than heappushpop when replacement is guaranteed.
