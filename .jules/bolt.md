...
## 2026-04-28 - Unreachable `__lt__` in heapq using `itertools.count()`
**Learning:** In Python's `heapq`, when tuples like `(priority, count, object)` are pushed, and `count` comes from an incrementing `itertools.count()`, the `count` acts as a unique, strictly increasing tie-breaker. Because no two elements will ever have the exact same `count`, the heap comparison logic will never evaluate the third element (`object`). Therefore, providing a custom `__lt__` and expensive timestamp operations inside `object` purely for tie-breaking is completely redundant and degrades performance without adding value.
**Action:** When using a unique incrementing integer as the secondary element in a heapq tuple, ensure that no custom `__lt__` methods or timestamp attributes are defined on the primary object for tie-breaking, as they are never called and only waste CPU cycles.
