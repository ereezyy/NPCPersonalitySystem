...
## 2026-04-24 - Hot path attribute access and LBYL vs EAFP
**Learning:** In intensely called hot paths like `Memory.add_event`, repeated instance attribute lookups (`self.events`, `len(self.events)`) and "Look Before You Leap" dictionary updates (`if tag not in dict`) add measurable overhead. Specifically, Python's EAFP (`try...except KeyError`) is significantly faster on the happy path than `.get()` or `if key in dict`. Furthermore, allocating an event object *before* doing capacity/importance rejection checks causes unneeded instantiation overhead.
**Action:** In high-frequency loop and memory capacity constraints: 1) Cache heavily accessed attributes like arrays or lengths into local variables. 2) Move rejection checks to the absolute beginning of the function to early exit before any objects or tuples are allocated. 3) Use EAFP `try...except KeyError` for conditional dictionary operations where the key usually exists or doesn't exist, as it avoids double lookups.
