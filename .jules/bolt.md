...
## 2024-05-25 - Defaultdict over explicit Dict initialization
**Learning:** Checking for key existence and manually creating default empty collections (`if tag not in dict: dict[tag] = set()`) inside hot path loops is an anti-pattern. `collections.defaultdict` executes this logic cleanly and efficiently in C, providing measurable performance improvements for repetitive key insertions and updates.
**Action:** Always prefer `collections.defaultdict` instead of explicit key existence checking when mapping keys to sets or lists inside frequently accessed insertion paths. Also, use `dict.get(key)` to retrieve elements safely rather than `dict[key]` if it's a `defaultdict` but you just want to read or safely remove it without unintentionally instantiating an empty collection.
