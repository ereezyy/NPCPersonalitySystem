...
## 2024-05-27 - Using heapreplace for guaranteed cache evictions
**Learning:** When managing a fixed-size priority queue with `heapq`, we often perform a check to see if a new item meets the threshold (e.g. `importance >= events[0][0]`) to avoid creating the object if it will just be rejected. Since we've already verified the new item is equal or greater than the minimum item, it is guaranteed that the new item will replace the minimum item. In this case, `heapq.heapreplace` is slightly faster than `heapq.heappushpop` because it skips an extra conditional check internally (it knows it doesn't need to return the *new* item).
**Action:** When maintaining a fixed-size min-heap in Python (e.g., using `heapq`), use `heapq.heapreplace` instead of `heapq.heappushpop` if it is guaranteed that the new item will replace an existing item (i.e., the new item is greater than or equal to the heap's minimum element), as it avoids unnecessary push-then-pop overhead.
