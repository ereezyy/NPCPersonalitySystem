...
## 2026-04-30 - Avoid unrolling dictionary iterations when optimizing hot paths
**Learning:** While unrolling a dictionary iteration manually (e.g., checking all 8 emotions by name) can yield minor performance gains, it creates a severe maintainability risk. If the dictionary keys change in the future, the unrolled loop will silently break. In Python, using `__slots__`, caching local variables (e.g., `emotions = self.emotions`), and using simple iteration (`for k in emotions: v = emotions[k]`) is often performant enough and far more robust than manually unrolling a loop for negligible gains.
**Action:** Avoid micro-optimizations that require manually repeating dictionary keys in logic. Use `__slots__` and local variable caching to speed up attribute lookups on classes that are instantiated frequently, and use `for key in dict: val = dict[key]` to avoid `.items()` view overhead while preserving dynamic keys.
